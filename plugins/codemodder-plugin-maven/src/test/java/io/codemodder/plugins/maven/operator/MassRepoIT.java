...
public class MassRepoIT {
    ...
    private static class TestRepo {
        ...
        public File cacheDir() {
            String cacheDirName = BASE_CACHE_DIR + File.separator + "repo-" + String.format("%08X", slug.hashCode());
            return new File(cacheDirName);
        }

        public static final File BASE_CACHE_DIR = new File(System.getProperty("user.dir") + "/.cache").getAbsoluteFile();
    }

    private final List<Pair<TestRepo, String>> repos = Arrays.asList(
            new Pair(
                    new TestRepo(
                            "WebGoat/WebGoat",
                            true,
    ...
    private String getDependenciesFrom(TestRepo repo) throws Exception {
        try {
            ...
            Collection<Dependency> dependencies = POMOperator.queryDependency(
                    POMScanner.scanFrom(
                                    pomFile,
                                    repo.cacheDir()
                            )
    ...
    private String getDependenciesFrom(String pomPath, File dir) throws IOException, InterruptedException {
        ...
        if (SystemUtils.IS_OS_WINDOWS) {
            command = new String[]{
                    Util.which("cmd").getCanonicalPath(),
                    "/c"
            };
        } else {
            command = new String[]{};
    ...
    private void testOnRepo(TestRepo sampleRepo, String dependencyToUpgradeString) throws Exception {
        ...
        ProjectModelFactory projectModelFactory = sampleRepo.useScanner ?
                POMScanner.scanFrom(
                        new File(sampleRepo.cacheDir(), sampleRepo.pomPath),
                        sampleRepo.cacheDir()
                ) :
                ProjectModelFactory.load(new File(sampleRepo.cacheDir(), sampleRepo.pomPath));

        ProjectModel context = projectModelFactory
        ...
        if (queryFailed) {
            ...
            Assert.assertFalse("Dependency should be originally missing", originalDependencies.contains(dependencyAsStringWithPackaging));
            Assert.assertTrue("New Dependency should be appearing", finalDependencies.contains(dependencyAsStringWithPackaging));
        }
    }
