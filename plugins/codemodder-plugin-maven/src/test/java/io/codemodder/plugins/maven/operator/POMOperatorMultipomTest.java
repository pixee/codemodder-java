...
public class POMOperatorMultipomTest extends AbstractTestBase {
    ...
    public void testWithParentAndChildMissingPackaging() throws Exception {
        ...
        ProjectModelFactory parentPom = ProjectModelFactory.load(parentResource)
                .withParentPomFiles(parentPomFiles);

        gwt("parent-and-child", parentPom.withDependency(Dependency.fromString("org.dom4j:dom4j:2.0.3")));
    }

    @Test(expected = WrongDependencyTypeException.class)
    public void testWithParentAndChildWrongType() throws Exception {
        File parentResource = getResource("parent-and-child-child-broken.xml");

        POMDocument parentPomFile = POMDocumentFactory.load(getResource("parent-and-child-parent.xml"));
        ...
        gwt("parent-and-child-wrong-type", parentPom.withDependency(Dependency.fromString("org.dom4j:dom4j:2.0.3")));
    }
    ...
    public void testWithMultiplePomsBasicNoVersionProperty() throws Exception {
        ...
        ProjectModelFactory projectModelFactory = ProjectModelFactory
                .load(getResource("sample-child-with-relativepath.xml"))
        ...
        ProjectModel result = gwt("multiple-pom-basic-no-version-property",
    ...
    public void testWithMultiplePomsBasicWithVersionProperty() throws Exception {
        ...
        ProjectModelFactory parentPom = ProjectModelFactory.load(sampleChild)
                .withParentPomFiles(Arrays.asList(POMDocumentFactory.load(parentPomFile)))
                .withUseProperties(true);

        ProjectModel result = gwt("multiple-pom-basic-with-version-property",
        ...
        String parentPomString = new String(result.getParentPomFiles().get(0).getResultPomBytes(), StandardCharsets.UTF_8);
    ...
    public void validateDepsFrom(ProjectModel context) throws Exception {
        ...
        resultFiles.entrySet().forEach(entry -> {
        ...
        Assert.assertTrue("Dependency " + context.getDependency() + " must be present in context " + context + " (" + dependencies + ")", foundDependency);
    }

    public Map<POMDocument, File> copyFiles(ProjectModel context) throws IOException, URISyntaxException {
        ...
        for (POMDocument p : context.getParentPomFiles()) {
            ...
            commonPath = new File(commonPath.getCanonicalPath()).toPath()
                    .relativize(new File(pCanonicalPath).toPath()).toFile();
        }
    ...
    public  Map<POMDocument, File> buildMap( ProjectModel context, int commonPathLen,  File tmpOutputDir) throws URISyntaxException, IOException {
        ...
        while(var10.hasNext()) {
            ...
            Intrinsics.checkNotNullExpressionValue(var21, "this as java.lang.String).substring(startIndex)");
