...
public abstract class SarifToLLMForMultiOutcomeCodemod extends SarifPluginRawFileChanger {
  ...
  private Optional<CodemodChange> processResult(
    ...
    try {
      ...
      Optional<CategorizeResponse> analysisOptional = categorize(file, result);
      if (analysisOptional.isEmpty()) {
        logger.debug("unable to categorize: {}", context.path());
        return Optional.empty();
      }
      final CategorizeResponse analysis = analysisOptional.get();
  ...
  private Optional<CategorizeResponse> categorize(final FileDescription file, final Result result) {
    ...
    int tokenCount = categorizationModel.tokens(List.of(systemMessage, userMessage));
    return tokenCount > categorizationModel.contextWindow() - 300
        ? Optional.empty()
        : Optional.of(getCategorizationResponse(systemMessage, userMessage));
  }
