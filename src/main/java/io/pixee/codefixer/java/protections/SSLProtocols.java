...
final class SSLProtocols {
    ...
    static Predicate<StringLiteralExpr> isUnsafeStringLiteral = new Predicate<StringLiteralExpr>() {
        @Override
    ...
    static Predicate<NameExpr> isUnsafeStringVariable = new Predicate<NameExpr>() {
        @Override
    ...
    static Predicate<ArrayCreationExpr> isUnsafeConstantStringArray = new Predicate<ArrayCreationExpr>() {
        @Override
    ...
    static Predicate<MethodCallExpr> hasUnsafeArrayArgumentVariable = new Predicate<MethodCallExpr>() {
        @Override
        public boolean test(final MethodCallExpr methodCallExpr) {
            ...
            if (holdingType != null) {
                ...
                if (field.isPresent()) {
                    ...
                    if (variables.size() == 1) {
                        VariableDeclarator variableDeclarator = variables.get(0);
                        final Type stringType1 = new ArrayType(new ClassOrInterfaceType("java.lang.String"));
                            ...
                            if (variableName.toString().equals(variableDeclarator.getName().toString())) {
                                if (variableDeclarator.getInitializer().isPresent()) {
                                    ...
                                    return variableExpression.isArrayCreationExpr()
                                            && isUnsafeConstantStringArray.test(variableExpression.asArrayCreationExpr());
    ...
    static Predicate<MethodCallExpr> hasUnsafeArrayArgument = new Predicate<MethodCallExpr>() {
        @Override
        public boolean test(final MethodCallExpr methodCallExpr) {
