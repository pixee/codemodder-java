...
final class VisitorAssemblerTest {
    ...
    void it_filters_java_factories_based_on_configuration() {
        ...
        RuleContext everythingButSecureRandom = RuleContext.of(DefaultRuleSetting.ENABLED, List.of("pixee:java/secure-random"));
        RuleContext everything = RuleContext.of(DefaultRuleSetting.ENABLED, List.of());
        File repositoryRoot = new File(".");
        List<VisitorFactory> allFactories = assembler.assembleJavaCodeScanningVisitorFactories(repositoryRoot, everything, List.of());
    ...
    void it_filters_file_visitors_based_on_configuration() {
        VisitorAssembler assembler = VisitorAssembler.createDefault();
        RuleContext everythingButDependencyInjection = RuleContext.of(DefaultRuleSetting.ENABLED, List.of("pixee:java/mvn-dependency-injection"));
        RuleContext everything = RuleContext.of(DefaultRuleSetting.ENABLED, List.of());
        List<FileBasedVisitor> allVisitors = assembler.assembleFileVisitors(everything);
        List<FileBasedVisitor> allButOne = assembler.assembleFileVisitors(everythingButDependencyInjection);
