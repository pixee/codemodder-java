...
test('If no includes/excludes, everything allowed', () => {
    let includesExcludes = fromConfiguration("src/test/repo/", [], []);
...
test('If only includes, should exclude everything else', () => {
    let includesExcludes = fromConfiguration("src/test/repo/", ["src/main/java"], []);
    const baseRepoPath = path.resolve("src/test/repo")
...
test('If only excludes, should include everything else', () => {
    let includesExcludes = fromConfiguration("src/test/repo/", [], ["src/main/java/acme/thing"]);
    const baseRepoPath = path.resolve("src/test/repo")
...
test('If including a single line in a file, ignore other lines', () => {
    let includesExcludes = fromConfiguration("src/test/repo/", ["src/main/java/acme/thing/thing.java:5"], []);
    const baseRepoPath = path.resolve("src/test/repo")
    expect(includesExcludes.shouldInspect(baseRepoPath + "/src/test/non_existent.java")).toBe(false);
