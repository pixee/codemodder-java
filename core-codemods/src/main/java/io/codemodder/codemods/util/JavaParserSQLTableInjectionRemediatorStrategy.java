...
import io.codemodder.codemods.SQLTableInjectionFilterTransform;
...
/**
 * Implementation of the JavaParserSQLInjectionRemediatorStrategy interface that deals with table
 * injections. This class provides the logic to visit a CompilationUnit and process findings for
 * potential SQL injections.
 */
public final class JavaParserSQLTableInjectionRemediatorStrategy
  ...
  public <T> CodemodFileScanningResult remediateAll(
    ...
    FixCandidateSearcher<T> searcher =
        new FixCandidateSearcher.Builder<T>()
            .withMatcher(SQLTableInjectionFilterTransform::matchCall)
            .build();
    ...
    for (FixCandidate<T> fixCandidate : results.fixCandidates()) {
      ...
      if (SQLTableInjectionFilterTransform.fix(methodCallExpr)) {
        issues.forEach(
            issue -> {
              final String id = findingIdExtractor.apply(issue);
